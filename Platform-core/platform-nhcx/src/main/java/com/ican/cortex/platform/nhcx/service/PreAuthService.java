...
import com.ican.cortex.platform.nhcx.fhir.bundle.ClaimBundleBuilder;
...
public class PreAuthService {
    ...
    private final ClaimBundleBuilder bundleBuilder;
    ...
    public Mono<SubmissionResponseDTO> submitPreAuth(PreAuthRequestDTO requestDTO) {
        String correlationId = UUID.randomUUID().toString();
        String apiCallId = UUID.randomUUID().toString();

        try {
            // 1. Build FHIR Bundle
            ClaimBundleBuilder.PreAuthRequestDTO bundleDto = new ClaimBundleBuilder.PreAuthRequestDTO();
            bundleDto.patientId = requestDTO.getPatientId();
            bundleDto.providerId = requestDTO.getProviderId();
            ...
            nhcxRequest.setRequestType("PreAuthorization");
            ...
            NhcxEvent auditEvent = NhcxEvent.builder()
                    ...
                    .apiCallId(apiCallId)
                    .type(NhcxEvent.EventType.PRE_AUTHORIZATION)
                    .status("INITIATED")
            ...
            return hcxWebClient.post()
                    .uri("/preauth/v1/submit") // Using v1 as per many specs, can be configured
                    .bodyValue(body)
                    .attributes(attrs -> {
            ...
            log.error("Error processing pre-auth submission for correlationId: {}", correlationId, e);
            return Mono.error(e);
        }
    }
}
